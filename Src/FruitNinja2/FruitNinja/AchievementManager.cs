...
// Type: FruitNinja2.AchievementManager
...
namespace FruitNinja2
{
  public class AchievementManager
  {
    ...
    {
      if (Game.isWP7TrialMode() && (Game.isWP7TrialMode() || TheGame.logInSucceeded))
        return;
      ...
      {
        if (TheGame.settings.aw <= 0 || AchievementManager.awarding)
          return;
        if ((TheGame.settings.aw & 1 << AchievementManager.achievementIndex) == 1 << AchievementManager.achievementIndex)
        {
    ...
    public static void UpdateUnlockDropdown(SpriteBatch batch)
    {
      ...
      if (AchievementManager.utype == 0)
        batch.DrawString(TheGame.instance.font3, TheGame.instance.stringTable.GetString(55), new Vector2(10f, (float) (AchievementManager.uy + 20)), Color.White);
      else
        batch.DrawString(TheGame.instance.font3, TheGame.instance.stringTable.GetString(54), new Vector2(10f, (float) (AchievementManager.uy + 20)), Color.White);
    }
    ...
    public static Texture2D GetPicture(int index)
    {
      lock (AchievementManager.achievementsLockObject)
      {
        if (AchievementManager.achievements != null)
        {
          ...
          return Texture2D.FromStream(TheGame.instance.graphics.GraphicsDevice, achievement.GetPicture());
        }
      }
    ...
    {
      if (Game.isWP7TrialMode() || !TheGame.logInSucceeded)
      {
        ...
        if (achievementIndex < 0)
          return;
        TheGame.settings.aw |= 1 << achievementIndex;
        TheGame.SaveConfig();
      }
      else
      {
    ...
    {
      SignedInGamer signedInGamer = default;//Gamer.SignedInGamers[(PlayerIndex) 0];
      if (signedInGamer == null || !TheGame.logInSucceeded)
        return;
      lock (AchievementManager.achievementsLockObject)
      {
        if (AchievementManager.achievements == null)
